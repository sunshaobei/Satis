...
import com.satis.scrollview.item.ListTypeBindingItem
import com.satis.scrollview.item.ListTypeItem
import com.satis.scrollview.recyclerview.ViewHolder
import com.satis.scrollview.recyclerview.wrapper.LoadMoreState

class SatisScrollView(val recyclerView: RecyclerView, datas: List<Any>) {
...
fun RecyclerView.satis(datas: List<Any>, block: SatisScrollView.() -> Unit):SatisScrollView {
    val satisScrollView = SatisScrollView(this, datas)
    satisScrollView.block()
    //设置layoutManager
    this.layoutManager = satisScrollView.layoutManager ?: layoutManager ?: LinearLayoutManager(this.context)
    //添加分割线
    satisScrollView.divider?.let {
        addItemDecoration(it)
    }
    adapter = satisScrollView.getAdapter()
    return satisScrollView
}